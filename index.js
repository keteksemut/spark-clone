...
const buildPath = (commands) =>
  commands.map((cmd) => `${cmd.type} ${cmd.points.join(" ")}`).join(" ");

const interpolPaths = (pathA, pathB, t) => {
  const cmdsA = parsePath(pathA);
  const cmdsB = parsePath(pathB);
  ...
  const interpoledCmds = cmdsA.map((cmdA, index) => {
    const cmdB = cmdsB[index];
    ...
    const interpoledPoints = cmdA.points.map((point, i) =>
      lerp(point, cmdB.points[i], t),
    );

    return {
      type: cmdA.type,
      points: interpoledPoints,
    };
  });
  return buildPath(interpoledCmds);
};
